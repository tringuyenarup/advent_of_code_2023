...
fn part_1(input: &str) -> Result<u16> {
    Ok(best_path::<1, 3>(input))
}

fn part_2(input: &str) -> Result<u16> {
    Ok(best_path::<4, 10>(input))
}
    ...
    fn pop_front(&mut self) -> Option<(usize, T)> {
        if self.first_filled >= self.items.len() {
            ...
            if self.items[self.first_filled].is_empty() {
                self.first_filled = self
                    .items
                    .iter()
                    .enumerate()
                    .skip(self.first_filled + 1)
                    .find_map(|(n, bucket)| if bucket.is_empty() { None } else { Some(n) })
                    .unwrap_or(usize::MAX)
            }
            ...
            for steps in 1..=MAX_STEPS {
                ...
                if steps >= MIN_STEPS {
                    ...
                    if tentative_g_score < old_g_score {
                        ...
                        frontier.push(
                            tentative_g_score as usize,
                            (new_direction, (new_x as u8, new_y as u8)),
                        )
                    }
                }
            }
